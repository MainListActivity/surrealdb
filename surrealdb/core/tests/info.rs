...
async fn permissions_checks_info_db() {
	...
	// Define the expected results for the check statement when the test statement
	// succeeded and when it failed
	let check = "{ accesses: {  }, analyzers: {  }, apis: {  }, buckets: {  }, configs: {  }, functions: {  }, models: {  }, modules: {  }, params: {  }, quota: { defined: false, generation: NONE }, sequences: {  }, tables: {  }, users: {  } }";
...
async fn access_info_redacted() {
	...
	// Record
	{
		...
		let out_expected =
			r#"{ accesses: { access: "DEFINE ACCESS access ON DATABASE TYPE RECORD WITH JWT ALGORITHM HS512 KEY '[REDACTED]' WITH ISSUER KEY '[REDACTED]' DURATION FOR TOKEN 1h, FOR SESSION NONE" }, analyzers: {  }, apis: {  }, buckets: {  }, configs: {  }, functions: {  }, models: {  }, modules: {  }, params: {  }, quota: { defined: false, generation: NONE }, sequences: {  }, tables: {  }, users: {  } }"#
				.to_string();
		let out_str = out.unwrap().to_sql();
		assert_eq!(
			out_str, out_expected,
...
WITH ISSUER KEY 'secret'; 			INFO FOR DB
		...
		let out_expected =
			r#"{ accesses: { access: "DEFINE ACCESS access ON DATABASE TYPE RECORD WITH REFRESH WITH JWT ALGORITHM HS512 KEY '[REDACTED]' WITH ISSUER KEY '[REDACTED]' DURATION FOR GRANT 4w2d, FOR TOKEN 1h, FOR SESSION NONE" }, analyzers: {  }, apis: {  }, buckets: {  }, configs: {  }, functions: {  }, models: {  }, modules: {  }, params: {  }, quota: { defined: false, generation: NONE }, sequences: {  }, tables: {  }, users: {  } }"#.to_string();
		let out_str = out.unwrap().to_sql();
		assert_eq!(
			out_str, out_expected,
			"Output '{out_str}' doesn't match expected output '{out_expected}'",
...
async fn access_info_redacted_structure() {
	...
	// Record
	{
		...
		let out_expected =
			r#"{ accesses: [{ duration: { session: 6h, token: 15m }, kind: { jwt: { issuer: { alg: 'HS512', key: '[REDACTED]' }, verify: { alg: 'HS512', key: '[REDACTED]' } }, kind: 'RECORD' }, name: 'access' }], analyzers: [], apis: [], buckets: [], configs: [], functions: [], models: [], modules: [], params: [], quota: { defined: false, generation: NONE }, sequences: [], tables: [], users: [] }"#
				.to_string();
		let out_str = out.unwrap().to_sql();
		assert_eq!(
			out_str, out_expected,
...
DURATION FOR GRANT 1w, FOR TOKEN 15m, FOR SESSION 6h; 			INFO FOR DB STRUCTURE
		...
		let out_expected =
			r#"{ accesses: [{ duration: { grant: 1w, session: 6h, token: 15m }, kind: { jwt: { issuer: { alg: 'HS512', key: '[REDACTED]' }, verify: { alg: 'HS512', key: '[REDACTED]' } }, kind: 'RECORD', refresh: true }, name: 'access' }], analyzers: [], apis: [], buckets: [], configs: [], functions: [], models: [], modules: [], params: [], quota: { defined: false, generation: NONE }, sequences: [], tables: [], users: [] }"#.to_string();
		let out_str = out.unwrap().to_sql();
		assert_eq!(
			out_str, out_expected,
			"Output '{out_str}' doesn't match expected output '{out_expected}'",
...
async fn function_info_structure() {
	...
	let out_expected =
		r#"{ accesses: [], analyzers: [], apis: [], buckets: [], configs: [], functions: [{ args: [['name', 'string']], block: "{ RETURN 'Hello, ' + $name + '!' }", name: 'example', permissions: true, returns: 'string' }], models: [], modules: [], params: [], quota: { defined: false, generation: NONE }, sequences: [], tables: [], users: [] }"#
		.to_string();
	let out_str = out.unwrap().to_sql();
	assert_eq!(
		out_str, out_expected,
